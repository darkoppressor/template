...
bool Button_Events::handle_button_event(string button_event,Window* parent_window){
    ...
    if(button_event.length()>0){
        if(button_event=="close_window"){
            ...
            if(parent_window!=0){
                network.edit_server(Engine::editing_server,parent_window->get_info_text(0),parent_window->get_info_text(1),Strings::string_to_unsigned_long(parent_window->get_info_text(2)),parent_window->get_info_text(3));
            ...
            Engine::configure_command=Strings::string_to_long(button_event);
